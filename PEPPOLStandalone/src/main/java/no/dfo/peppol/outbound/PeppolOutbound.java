...
public class PeppolOutbound {
	...
	public boolean PEPPOL_Outbound(UtilityValuesGUI uvg) throws Exception {
		for (int i = 0; i < uvg.getISOFiles().size(); i++) {
			...
			if (orgnr == null)
				throw new RuntimeException("'BIC-Orgnr' not found");
			...
			inMemoryDocument1.setMimeType(appXml);
			...
			inMemoryDocument2.setMimeType(appXml);
			...
			try (InputStream isInnerAsic = signer.signASiCSBaselineB(docsToSign, messageId, uvg, uv);) {
				...
				if (isInnerAsic.available() > 1000) {
					...
					throw new RuntimeException("cloneStandardBusinessDocument() - error signing inner asic");
				}
				File ZipPackageInner = new File(uvg.getRepackPath() + File.separator + uv.getFilename() + File.separator
						+ uv.getFilename() + "_inner.zip");
				Log.info("[" + uv.getFilename() + "_" + uvg.getTimestamp() + "]"
						+ "Outer ASiC : Packaging End : sbdh.xml File Created successfully ");
				...
				inMemoryDocument3.setMimeType(appXml);
				...
				inMemoryDocument4.setMimeType(appXml);
				...
				try (InputStream isOuterAsic = signer.signASiCSBaselineB(docsToSign, messageId, uvg, uv);) {
					if (isOuterAsic.available() > 1000) {
						...
						File SBDFile = new File(uvg.getRepackPath() + File.separator + uv.getFilename() + File.separator
								+ uvg.getSenderOrg()+"_"+uvg.getRecOrg()+"_" +currentDateString+"_"+ uv.getFilename());
						FileUtils.writeStringToFile(SBDFile, newSbdh);
						File ZipPackage = new File(uvg.getRepackPath() + File.separator + uv.getFilename()
								+ File.separator + uv.getFilename() + "_outer.zip");
						Log.info("[" + uv.getFilename() + "_" + uvg.getTimestamp() + "]"
								+ "Outer ASiC : Packaging End : sbd.xml File Created successfully ");
						...
						throw new SignatureValidationErrorOuter("error in signing outer asic");
				...
				throw new SignatureValidationErrorInner("cloneStandardBusinessDocument() - error signing inner asic");
